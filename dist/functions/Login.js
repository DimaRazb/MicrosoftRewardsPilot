...
const human_behavior_1 = require("../src/anti-detection/human-behavior");
...
class Login {
    constructor(bot) {
        ...
        this.bot = bot;
        this.humanBehavior = new human_behavior_1.HumanBehaviorSimulator();
    }
    ...
    async startSignInFromWelcome(page) {
        try {
     ...
     */
    async handlePostPasswordTwoFactor(page) {
        try {
    ...
    async authEmailVerification(page) {
    ...
    async tryAlternative2FAMethods(page) {
        try {
    ...
    async getMobileAccessToken(page, email) {
        ...
        while (true) {
            ...
            if (elapsed > maxWaitMs) {
                throw new Errors_1.LoginTimeoutError(`OAuth authorization timeout after ${Math.round(elapsed / 1000)}s`);
            }
        }
    ...
    async dismissLoginMessages(page) {
    ...
    async tryDismissPasskeyPrompt(page) {
        try {
    ...
    async checkAccountLocked(page) {
        ...
        if (isLocked) {
            ...
            throw new Errors_1.AccountLockedError('This account has been locked! Remove the account from "accounts.json" and restart!');
        }
    }
    async handleMobile2FAPage(page) {
        try {
            const currentUrl = page.url();
